...
#define RIME_VARIANT_DEFINE_UNARY_OPERATOR(name, operation)                   \
    namespace variant_detail {                                                \
        ...
        {                                                                     \
            ...
            decltype(operation std::declval<Actual &&>()) operator()(         \
                Actual && actual) const                                       \
            {                                                                 \
        ...
        {                                                                     \
            typedef decltype(visit(name())(std::declval<Variant &&>())) type; \
        };                                                                    \
    } /* namespace variant_detail */                                          \
                                                                              \
    template <class Variant> inline typename boost::enable_if<                \
        is_variant<Variant>,                                                  \
        typename variant_detail::name##_result<Variant, void>::type>::type    \
        operator operation(Variant && variant)                                \
    {                                                                         \
        return visit(variant_detail::name())(std::forward<Variant>(variant)); \
    }
...
#define RIME_VARIANT_DEFINE_UNARY_POSTFIX_OPERATOR(name, operation)           \
    namespace variant_detail {                                                \
        ...
        {                                                                     \
            template <typename Actual>                                        \
            decltype(std::declval<Actual &&>() operation) operator()(         \
                Actual && actual) const                                       \
            {                                                                 \
    ...
    template <class Variant> inline typename boost::enable_if<                \
        is_variant<Variant>,                                                  \
        typename variant_detail::name##_result<Variant>::type>::type          \
        operator operation(Variant && variant, int)                           \
    {                                                                         \
        return visit(variant_detail::name())(std::forward<Variant>(variant)); \
    }
