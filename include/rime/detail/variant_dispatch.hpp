...
namespace variant_detail {
    ...
    When we know the actual type of a parameter, we need to convert it to that
    type. In the case of a non-variant type, say int a, both the argument type
    and the actual type are int. In the case of variant <char, long> b, say the
    actual type is char, then we need to call get <char>(b).

    Convert one type to its actual type as returned by get if it is a variant,
    and the original type if it is not.
    */
    template <typename Argument, typename Actual> inline
        typename boost::enable_if<
            std::is_same<Argument, Actual>, Actual &&>::type
        convert_to_actual(Argument && argument)
    {
        return std::forward<Argument>(argument);
    }

    template <typename Variant, typename Actual> inline
        typename boost::enable_if<
            is_variant<Variant>,
    ...
    */
    ...
    // Implementations for actual arguments containing void, and not,
    // respectively.
    template <typename Arguments, typename ActualArguments>
    ...
    template <
        typename Function, typename... Arguments, typename ActualFunction,
        typename... ActualArguments>
    struct dispatch_recipient_no_void<
        ...
        meta::vector<ActualFunction, ActualArguments...>>
    {
        ...
        */
        typedef decltype(convert_to_actual<Function, ActualFunction>(
        ...
        The error happens during the instantiation of this class, which you
        should be able to find in the error message (hopefully somwhere at the
        top). Say it is rime::variant_detail::dispatch_recipient_no_void <
                meta::vector <double (&)(int, float), rime::variant <int, char>
        >, meta::vector <double (&)(int, float), int> >. The first meta::vector
        indicates the compile-time function call, including variant types, and
        the second meta::vector the run-time function call, with the actual
        types. The first type, here "double (&)(int, float)", is the function
        type. The rest are the parameters to this function.

        Note that if you passed in arguments that can be void, even if they are
        not void at run time, then they may have been removed from the list of
        parameters, and they will be invisible here.
        In this example, the second parameter was variant <void, float>, which
        got removed to implement the case where at run time it contains void. In
        the above example, the function obviously takes two parameters, and
        receives only one.
    ...
    template <
        typename MergedResultType, class DispatchRecipient,
    ...
    */
    ...
    struct convert_result_impl : DispatchRecipient
    {};
    ...
    /**
    3. List all possible combinations of actual types.
    Represent the list of all possible combinations of actual types that a list
    of types including variants can be. This list is at the basis of multiple
    dispatch. For example, possible_actual_types_for <meta::vector < variant
    <int, long>, variant <bool, double> > > will have as type: meta::vector <
    ...
    private:
        ...
        */
        template <typename Type> static
            typename boost::disable_if<is_variant<Type>, std::size_t>::type
            which(Type &&)
        {
            return 0;
        };
        template <typename Type> static
            typename boost::enable_if<is_variant<Type>, std::size_t>::type
            which(Type && variant)
        {
    ...
    public:
        ...
        */
        typedef typename meta::flatten<typename meta::as_vector<meta::transform<
            types_starting_with<boost::mpl::_1>, type_first>>::type>::type type;
...
template <typename Function> class visitor
{
    ...
    */
    template <
        ...
        typename PossibleRecipients = typename meta::as_vector<meta::transform<
    ...
    template <
        typename... Arguments, typename PossibleActualArguments,
...
public:
    ...
    template <typename... Arguments> typename variant_dispatcher<
        meta::vector<Function, Arguments...>>::result_type
        operator()(Arguments &&... arguments)
    {
        variant_dispatcher<meta::vector<Function, Arguments...>> dispatcher;
        return dispatcher(
            std::forward<Function>(function),
            std::forward<Arguments>(arguments)...);
    }
...
namespace variant_detail {
    ...
    /*
    Implementation of dispatch_recipient that can deal with actual types being
    void. They get removed from the argument list.
    */
    ...
    */
    ...
    template <
        typename FirstArgument, typename... Arguments, typename FirstActual,
        typename... Actual>
    class split_void<
    ...
    public:
        ...
        */
        typedef typename meta::push<
            FirstArgument, typename recursive_split::before_arguments>::type
            before_arguments;
        /**
        ...
        */
        typedef typename meta::push<
            FirstActual, typename recursive_split::new_actual>::type new_actual;
    };
    ...
    template <
        class Arguments, class Actual, class SeenArguments, class SeenActual>
    ...
    */
    template <
        class Arguments, class Actual, class SplitVoid,
        typename... BeforeArguments, typename... AfterArguments>
    class dispatch_recipient_remove_void_fast<
        Arguments, Actual, SplitVoid, meta::vector<BeforeArguments...>,
        meta::vector<AfterArguments...>>
    {
    ...
    */
    template <
        typename FirstArgument, typename... Arguments, typename FirstActual,
        typename... Actual, typename... SeenArguments, typename... SeenActual>
    class dispatch_recipient_remove_void_slow<
        meta::vector<FirstArgument, Arguments...>,
        meta::vector<FirstActual, Actual...>, meta::vector<SeenArguments...>,
        meta::vector<SeenActual...>>
    {
        typedef dispatch_recipient_remove_void_slow<
            ...
            meta::vector<SeenActual..., FirstActual>>
            recursive_type;
    ...
    template <
        typename FirstArgument, typename... Arguments, typename... Actual,
        typename... SeenArguments, typename... SeenActual>
    class dispatch_recipient_remove_void_slow<
        meta::vector<FirstArgument, Arguments...>,
        meta::vector<void, Actual...>, meta::vector<SeenArguments...>,
        meta::vector<SeenActual...>>
    {
        typedef dispatch_recipient_remove_void_slow<
            meta::vector<Arguments...>, meta::vector<Actual...>,
    ...
    struct dispatch_recipient
    : boost::mpl::if_<
          meta::contains<void, ActualArguments>,
          dispatch_recipient_remove_void<Arguments, ActualArguments>,
          dispatch_recipient_no_void<Arguments, ActualArguments>>::type
    {};
