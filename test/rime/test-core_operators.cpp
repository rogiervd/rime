...
void check_constant_equals(
    rime::constant<Type, Content> const &, ReferenceType const & reference)
{
...
#define TEST_BINARY_OPERATOR_ON_TYPE(TYPE, NAME, OPERATOR, LEFT, RIGHT)        \
    {                                                                          \
        ...
        check_constant_equals(                                                 \
            rime::NAME(mpl_left, right), (LEFT OPERATOR RIGHT));               \
        check_constant_equals(                                                 \
            rime::NAME(left, mpl_right), (LEFT OPERATOR RIGHT));               \
        check_constant_equals(                                                 \
            rime::NAME(mpl_left, mpl_right), (LEFT OPERATOR RIGHT));           \
        ...
        /* Infect with MPL constant: return compile-time constant */           \
        check_constant_equals(                                                 \
            (mpl_left OPERATOR right), (LEFT OPERATOR RIGHT));                 \
        check_constant_equals(                                                 \
            (left OPERATOR mpl_right), (LEFT OPERATOR RIGHT));                 \
        /* Two MPL constants: no change to behaviour, return runtime value */  \
        RIME_CHECK_EQUAL(                                                      \
            (mpl_left OPERATOR mpl_right), (LEFT OPERATOR RIGHT));             \
        /* Infect with runtime value: return runtime value */                  \
...
#define TEST_BINARY_OPERATOR_ON_FLOAT(NAME, OPERATOR, LEFT, RIGHT)           \
    {                                                                        \
        ...
        /* Explicit function */                                              \
        RIME_CHECK_EQUAL(                                                    \
            rime::NAME(left, double(RIGHT)), (LEFT OPERATOR double(RIGHT))); \
        RIME_CHECK_EQUAL(                                                    \
            rime::NAME(double(LEFT), right), (double(LEFT) OPERATOR RIGHT)); \
        RIME_CHECK_EQUAL(                                                    \
            rime::NAME(double(LEFT), double(RIGHT)),                         \
            (double(LEFT) OPERATOR double(RIGHT)));                          \
                                                                             \
        /* Operator with constant */                                         \
        RIME_CHECK_EQUAL(                                                    \
            (left OPERATOR double(RIGHT)), (LEFT OPERATOR double(RIGHT)));   \
        RIME_CHECK_EQUAL(                                                    \
            (double(LEFT) OPERATOR right), (double(LEFT) OPERATOR RIGHT));   \
    }
