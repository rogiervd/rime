...
BOOST_AUTO_TEST_CASE(test_rime_variant_assign)
{
    ...
    // Assign with objects and moving
    {
        ...
        using utility::tracked;
        {
            ...
            BOOST_CHECK_EQUAL(
                registry.counts(),
                value_construct_count(2) + move_count(1)
...
BOOST_AUTO_TEST_CASE(test_rime_make_variant_over)
{
    ...
    BOOST_MPL_ASSERT(
        (std::is_same<make_variant_over<meta::vector<int>>::type, int>) );

    BOOST_MPL_ASSERT(
        (std::is_same<make_variant_over<meta::vector<int, int>>::type, int>) );
    ...
    BOOST_MPL_ASSERT(
        (std::is_same<
            make_variant_over<meta::vector<int, rime::int_<10>>>::type, int>) );
    ...
    BOOST_MPL_ASSERT(
        (rime::same_constant<rime::constant<int, 10>, rime::int_<10>>) );
    ...
    BOOST_MPL_ASSERT((std::is_same<
                      make_variant_over<meta::vector<
                          rime::false_type, rime::constant<bool, false>>>::type,
...
template <typename... Types, typename Argument> void check_same_type_equal(
...
template <typename... Types, typename Argument> void check_contains(
...
BOOST_AUTO_TEST_CASE(test_variant_operator_unary)
{
    ...
    // Operators that can be tested on pointer values.
    {
        ...
        typedef rime::variant<
            unsigned *, std::shared_ptr<int>, std::unique_ptr<long>>
            variant;
