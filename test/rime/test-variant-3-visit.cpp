...
{
public:
    template <typename Argument> Argument operator()(Argument argument) const
    {
        return argument;
    }
...
public:
    ...
    template <class Argument> void operator()(Argument && argument) const
    {
...
public:
    ...
    template <typename Argument> Argument && operator()(Argument && left) const
    {
        return left;
    }
...
struct plus_assign_void
{
    ...
    template <typename Argument> void operator()(Argument) const {}
...
BOOST_AUTO_TEST_CASE(test_rime_variant_visit)
{
    ...
    }
    {
        ...
        BOOST_CHECK_EQUAL(rime::get<int>(v1), 9);
        BOOST_CHECK_EQUAL(&rime::get<int &>(result), &rime::get<int>(v1));
    ...
    }
    {
        ...
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(int_, int_, int_)),
            3);
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(void_, int_, int_)),
            2);
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(int_, void_, int_)),
            2);
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(int_, int_, void_)),
            2);
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(int_, void_, void_)),
            1);
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(void_, int_, void_)),
            1);
        BOOST_CHECK_EQUAL(
            rime::get<int>(rime::visit(count_arguments())(void_, void_, int_)),
            1);
...
struct return_one_from_last : non_copyable
{
    ...
    typename std::add_pointer<Type2>::type operator()(
        Type1 &&, Type2 && result, Type3 &&) const
    {
        return &result;
    }
};
...
BOOST_AUTO_TEST_CASE(test_rime_variant_call)
{
    ...
    {
        rime::variant<plus, plus_assign_void, count_arguments> v_plus((plus()));
    ...
    // Test that there is no gratuitous copying.
    {
        ...
        n2 = rime::visit(f)(rime::variant<int, bool>(3), std::move(n), 'a');
        BOOST_CHECK(&n == n2);

        n2 = rime::visit(f)(rime::variant<int, bool>(3), n, 'a');
        BOOST_CHECK(&n == n2);
    }
}

double take_two_arguments(int a, float b) { return double(a) + b; }
    ...
    */
    /*
        ...
        double d1 = rime::visit (take_two_arguments) (
            rime::variant <int, char> ('a'), 4.f, rime::variant <int, float>
       (4.f)); double d2 = rime::visit (take_two_arguments) ( rime::variant
       <int, char> ('a'), rime::variant <void, float> (4.f));
    */
}
