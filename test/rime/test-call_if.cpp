...
BOOST_AUTO_TEST_CASE(test_rime_call_if_merge)
{
    ...
    typedef merge_policy::to_variant<merge_policy::reference<
        merge_policy::const_<merge_float<merge_policy::same<>>>>>
        policy;

    auto v1 =
        rime::call_if<policy>(false, return_as_double(), return_as_long(), 2);
    BOOST_CHECK(v1.contains<long>());
    BOOST_CHECK_EQUAL(rime::get<long>(v1), 2);

    auto v2 =
        rime::call_if<policy>(false, return_as_double(), return_as_float(), 7);
