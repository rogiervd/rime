...
BOOST_AUTO_TEST_CASE(test_rime_sign_types)
{
    ...
    static_assert(
        rime::types::is_signed<std::integral_constant<int, 1>>::value, "");
    ...
    static_assert(
        !rime::types::is_unsigned<std::integral_constant<int, 1>>::value, "");
    ...
    // make_signed.
    BOOST_MPL_ASSERT(
        (std::is_same<typename rime::types::make_signed<int>::type, int>) );
    BOOST_MPL_ASSERT((
        std::is_same<typename rime::types::make_signed<unsigned>::type, int>) );
    BOOST_MPL_ASSERT(
        (std::is_same<typename rime::types::make_signed<short>::type, short>) );
    BOOST_MPL_ASSERT(
        (std::is_same<
    ...
    BOOST_MPL_ASSERT((std::is_same<
                      typename rime::types::make_unsigned<short>::type,
                      short unsigned>) );
    BOOST_MPL_ASSERT((std::is_same<
    ...
    // make_zero.
    RIME_CHECK_EQUAL(
        rime::types::make_zero<rime::int_<1>>::type(), rime::int_<0>());
    RIME_CHECK_EQUAL(
        rime::types::make_zero<rime::int_<-5>>::type(), rime::int_<0>());
    RIME_CHECK_EQUAL(
...
BOOST_AUTO_TEST_CASE(test_rime_sign_convert)
{
    ...
    RIME_CHECK_EQUAL(
        rime::make_unsigned(rime::int_<3>()), (rime::constant<unsigned, 3u>()));
    RIME_CHECK_EQUAL(
        rime::make_unsigned(rime::constant<unsigned, 2u>()),
        (rime::constant<unsigned, 2u>()));
...
static_assert(
    int(very_great_unsigned::value) < 0,
...
void check_less_sign_safe(Left const & left, Right const & right)
{
    ...
    RIME_CHECK_EQUAL(
        rime::less_sign_safe(left, right), rime::bool_<reference>());
    ...
    RIME_CHECK_EQUAL(
        rime::less_sign_safe(Left::value, Right::value), reference);
}

// The case where if the left operand is constant, the result is known.
// I.e. -1 < u for any unsigned u.
template <bool reference, class Left, class Right>
void check_less_sign_safe_left_constant(Left const & left, Right const & right)
{
    // Known at compile time.
    RIME_CHECK_EQUAL(
        rime::less_sign_safe(left, right), rime::bool_<reference>());
    RIME_CHECK_EQUAL(
        rime::less_sign_safe(left, Right::value), rime::bool_<reference>());
...
template <bool reference, class Left, class Right>
void check_less_sign_safe_right_constant(Left const & left, Right const & right)
{
    // Known at compile time.
    RIME_CHECK_EQUAL(
        rime::less_sign_safe(left, right), rime::bool_<reference>());
    RIME_CHECK_EQUAL(
        rime::less_sign_safe(Left::value, right), rime::bool_<reference>());
